...
(() => {
  ...
  // src/features/glossaryTable/scroll.ts
  ...
  function closestTarget(e) {
    var _a, _b;
    const el = e.target;
    return (_b = (_a = el == null ? void 0 : el.closest) == null ? void 0 : _a.call(el, ".glossaryTerm[data-gt-target]")) != null ? _b : null;
  }
  ...
  function extractLibrary(hostname) {
    ...
    return (_b = (_a = parts == null ? void 0 : parts[0]) == null ? void 0 : _a.toLowerCase()) != null ? _b : "dev";
  }

  // src/config.ts
  var API_HOST = "https://option-shelf-animals-destiny.trycloudflare.com";
  ...
  function unescapeLatex(str) {
    ...
    do {
      prev = s;
      s = s.replace(/\\\\\(/g, "\\(").replace(/\\\\\)/g, "\\)").replace(/\\\\\[/g, "\\[").replace(/\\\\\]/g, "\\]").replace(/\\\\%/g, "\\%").replace(/\\\\#/g, "\\#").replace(/\\\\&/g, "\\&").replace(/\\\\_/g, "\\_").replace(/\\\$/g, "$");
    } while (s !== prev);
    s = s.replace(
      /\\\(([\s\S]*?)\\\)/g,
      (_m, inner) => "\\(" + inner.replace(/(^|[^\\])%/g, (_mm, prefix) => prefix + "\\%") + "\\)"
    );
    return s;
  }
  ...
  function inLatexRange(index, len, ranges) {
    const end2 = index + len;
    return ranges.some(([start2, rangeEnd]) => index < rangeEnd && end2 > start2);
  }
  ...
  function triggerMathJax(target) {
    ...
    function runTypeset() {
      ...
      if (mj.loader && typeof mj.loader.load === "function") {
        Promise.resolve(mj.loader.load("[tex]/mhchem")).then(() => {
  ...
  function typesetTooltip(el, done) {
    ...
    function run() {
      ...
      try {
        (_b2 = (_a2 = window.MathJax).typesetClear) == null ? void 0 : _b2.call(_a2, [target]);
      } catch (e) {
      }
      window.MathJax.typesetPromise([target]).then(() => done == null ? void 0 : done()).catch((err) => {
    ...
    if (typeof window.MathJax === "undefined" || !window.MathJax.typesetPromise) {
      ...
      const poll = setInterval(() => {
        ...
        if (typeof window.MathJax !== "undefined" && window.MathJax.typesetPromise) {
          clearInterval(poll);
          const ready2 = (_b2 = (_a2 = window.MathJax.startup) == null ? void 0 : _a2.promise) != null ? _b2 : Promise.resolve();
          ready2.then(run).catch(run);
        } else if (++attempts > 50) {
          clearInterval(poll);
        }
      }, 100);
    ...
    const ready = (_b = (_a = window.MathJax.startup) == null ? void 0 : _a.promise) != null ? _b : Promise.resolve();
  ...
  function buildTooltipHTML(item) {
    ...
    if (hasValue(item.author)) {
      termAttrParts.push(
        '<p class="gt-attr-row"><span class="gt-attr-label">Author: </span>' + escapeHTML(item.author) + "</p>"
      );
    }
    if (hasValue(item.source)) {
      const licenseText = item.source && LICENSE_MAP[item.source] || item.source || "";
      termAttrParts.push(
        '<p class="gt-attr-row"><span class="gt-attr-label">License: </span>' + escapeHTML(licenseText) + "</p>"
      );
    }
    if (hasValue(item.aliases) && ((_b = (_a = item.aliases) == null ? void 0 : _a.length) != null ? _b : 0) > 0) {
      termAttrParts.push(
        '<p class="gt-attr-row"><span class="gt-attr-label">Aliases: </span>' + escapeHTML(item.aliases.join(", ")) + "</p>"
      );
    }
    if (hasValue(item.link)) {
      termAttrParts.push(
        '<a class="gt-link" href="' + escapeHTML(item.link) + '" target="_blank" rel="noopener" aria-label="Read more about ' + term + ' (opens in new tab)">Read more <span aria-hidden="true">&rarr;</span></a>'
      );
    }
    ...
    const tabButtons = tabDefs.map(
      (t, i) => '<button class="gt-tab' + (i === 0 ? " gt-tab--active" : "") + '" role="tab" id="' + t.btnId + '" aria-selected="' + (i === 0 ? "true" : "false") + '" aria-controls="' + t.id + '" tabindex="' + (i === 0 ? "0" : "-1") + '" onclick="_gtSwitchTab(this,' + i + ')" onkeydown="_gtTabKeydown(event,this,' + i + "," + total + ')">' + t.label + "</button>"
    ).join("");
    const tabPanels = tabDefs.map(
      (t, i) => '<div class="gt-panel' + (i === 0 ? " gt-panel--active" : "") + '" role="tabpanel" id="' + t.id + '" aria-labelledby="' + t.btnId + '"' + (i !== 0 ? ' aria-hidden="true"' : "") + ">" + t.content + "</div>"
    ).join("");
    return '<div class="gt-tooltip"><div class="gt-tabs" role="tablist" aria-label="' + term + ' sections">' + tabButtons + "</div>" + tabPanels + "</div>";
  }

  // src/tooltip/styles.ts
  var TIPPY_BASE_CSS = '.tippy-box[data-animation=fade][data-state=hidden]{opacity:0}[data-tippy-root]{max-width:calc(100vw - 10px)}.tippy-box{position:relative;background-color:#333;color:#fff;border-radius:4px;font-size:14px;line-height:1.4;white-space:normal;outline:0;transition-property:transform,visibility,opacity}.tippy-box[data-placement^=top]>.tippy-arrow{bottom:0}.tippy-box[data-placement^=top]>.tippy-arrow:before{bottom:-7px;left:0;border-width:8px 8px 0;border-top-color:initial;transform-origin:center top}.tippy-box[data-placement^=bottom]>.tippy-arrow{top:0}.tippy-box[data-placement^=bottom]>.tippy-arrow:before{top:-7px;left:0;border-width:0 8px 8px;border-bottom-color:initial;transform-origin:center bottom}.tippy-box[data-placement^=left]>.tippy-arrow{right:0}.tippy-box[data-placement^=left]>.tippy-arrow:before{border-width:8px 0 8px 8px;border-left-color:initial;right:-7px;transform-origin:center left}.tippy-box[data-placement^=right]>.tippy-arrow{left:0}.tippy-box[data-placement^=right]>.tippy-arrow:before{left:-7px;border-width:8px 8px 8px 0;border-right-color:initial;transform-origin:center right}.tippy-box[data-inertia][data-state=visible]{transition-timing-function:cubic-bezier(.54,1.5,.38,1.11)}.tippy-arrow{width:16px;height:16px;color:#333}.tippy-arrow:before{content:"";position:absolute;border-color:transparent;border-style:solid}.tippy-content{position:relative;padding:5px 9px;z-index:1}';
  var GT_STYLES = [
  ...
  function attachTooltips(root = document, selector = ".glossary-term") {
    ...
    tippy_esm_default(targets, {
      ...
      aria: {
        content: "describedby",
        // sets aria-describedby on the button while open
        expanded: "auto"
      },
      content(el) {
        try {
          const raw = el.dataset.gtItem;
          return raw ? buildTooltipHTML(JSON.parse(raw)) : "";
        } catch (e) {
          return "";
        }
      },
      ...
      onMount(instance) {
        typesetTooltip(instance.popper, () => {
          var _a, _b;
          (_a = instance.popperInstance) == null ? void 0 : _a.update();
  ...
  // src/features/glossarize/run.ts
  var activeTermMap = null;
  function getActiveTermMap() {
  ...
  // src/features/glossarize/watch.ts
  var DEBOUNCE_MS = 200;
  var OWN_UI_SELECTOR = ".glossary-term, .tippy-box, [data-tippy-root], #gt-lightbox";
  function nodeRoot(node, contentRoot) {
  ...
  function watchForContentChanges() {
    var _a;
    const contentRoot = (_a = document.querySelector(".mt-content-container")) != null ? _a : document.body;
  ...
  function openLightbox(src, alt, caption) {
    var _a, _b, _c;
    (_a = document.getElementById("gt-lightbox")) == null ? void 0 : _a.remove();
    ...
    overlay.innerHTML = '<div class="gt-lb-backdrop"></div><div class="gt-lb-dialog"><button class="gt-lb-close" onclick="_gtCloseLightbox()" aria-label="Close enlarged image">&times;</button><img class="gt-lb-img" src="' + src + '" alt="' + (alt || "") + '" />' + (caption ? '<p class="gt-lb-caption">' + caption + "</p>" : "") + "</div>";
    (_b = overlay.querySelector(".gt-lb-backdrop")) == null ? void 0 : _b.addEventListener("click", closeLightbox);
    window._gtLbKeyHandler = (e) => {
      if (e.key === "Escape") {
        closeLightbox();
        return;
      }
    ...
    (_c = overlay.querySelector(".gt-lb-close")) == null ? void 0 : _c.focus();
  }
  function closeLightbox() {
    ...
    const trigger = window._gtLbTrigger;
  ...
  function updateTippyPopper(el) {
    var _a, _b, _c;
    const box = (_a = el.closest) == null ? void 0 : _a.call(el, ".tippy-box");
    (_c = (_b = box == null ? void 0 : box._gtTippy) == null ? void 0 : _b.popperInstance) == null ? void 0 : _c.update();
  }
  function switchTab(btn, idx) {
    const tooltip = btn.closest(".gt-tooltip");
    if (!tooltip) return;
    tooltip.querySelectorAll(".gt-tab").forEach((b, i) => {
      const active = i === idx;
      b.classList.toggle("gt-tab--active", active);
      b.setAttribute("aria-selected", active ? "true" : "false");
      b.setAttribute("tabindex", active ? "0" : "-1");
    });
    tooltip.querySelectorAll(".gt-panel").forEach((p, i) => {
      const active = i === idx;
      p.classList.toggle("gt-panel--active", active);
      p.setAttribute("aria-hidden", active ? "false" : "true");
    });
    const panel = tooltip.querySelectorAll(".gt-panel")[idx];
    if (panel) {
      typesetTooltip(panel, () => updateTippyPopper(tooltip));
    }
  }
  function tabKeydown(e, btn, idx, total) {
    const tooltip = btn.closest(".gt-tooltip");
    if (!tooltip) return;
    const tabs = Array.from(tooltip.querySelectorAll(".gt-tab"));
    let newIdx = -1;
...
//# sourceMappingURL=glossarizer.js.map
